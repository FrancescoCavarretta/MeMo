    ...
    for lesioned_flag in [ True, False ]:
        ...
        for cellid in cellids:
            for g_drv, n_drv, n_bg, n_rtn in param_space:
                ...
                if lesioned_flag:
                    key += '-6ohda'            
                params.append({'cellid':cellid, 'seed':seed, 'n_drv':n_drv, 'n_mod':n_mod, 'n_bg':n_bg, 'n_rtn':n_rtn, 'g_drv':g_drv, 'lesioned_flag':lesioned_flag, 'tstop':1000, 'key':key})
                                        
print (len(params))
np.save('selected_cfg_test.npy', params, allow_pickle=True)
print ('done')
